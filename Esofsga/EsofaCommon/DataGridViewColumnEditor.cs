        ...
        public void ColumHeaderEdit(DataGridView dgv, string dgvName)
        {
            ...
            if (dgvName == "dgvTarget")
            {
                string[] tgtHeaderTxt = new string[] { "目标区", "盆地/区域 ", "主力层系", "保存条件", "最小_地质资源量 (万亿方)",
                    ...
                    "干酪根类型","最小_Ro(%)","最大_Ro(%)","有效圈定面积(1000-4000米深,平方千米)","最小_含气量 (立方米/吨)",
                    "最大_含气量 (立方米/吨)","最小_资源丰度(亿立方米/平方千米)","最大_资源丰度(亿立方米/平方千米)","最小_孔隙度(%)",
            ...
            if (dgvName == "dgvBasin")
            {
                string[] bsnHeaderTxt = new string[] { "目标区", "盆地/区域 ", "主力层系", "保存条件", "最小_地质资源量 (万亿方)",
                    "最大_地质资源量 (万亿方)","最小_富有机质页岩厚度","最大_富有机质页岩厚度","最小_TOC(%)","最大_TOC(%)",
                    "干酪根类型","最小_Ro(%)","最大_Ro(%)","有效圈定面积(1000-4000米深,平方千米)","最小_含气量 (立方米/吨)",
                    "最大_含气量 (立方米/吨)","最小_资源丰度(亿立方米/平方千米)","最大_资源丰度(亿立方米/平方千米)","最小_孔隙度(%)",
                    "最大_孔隙度(%)","构造复杂度","顶底板岩性","最小_埋深范围","最大_埋深范围","最小_压力系数","最大_压力系数",
            ...
            if (dgvName == "dgvView")
            {
                string[] viewHeaderTxt = new string[] { "目标区", "盆地/区域 ", "主力层系", "保存条件", "平均_地质资源量 (万亿方)",
                    "平均_富有机质页岩厚度", "平均_TOC(%)","干酪根类型","平均_Ro(%)","有效圈定面积(1000-4000米深,平方千米)",
                    "平均_含气量 (立方米/吨)","平均_资源丰度(亿立方米/平方千米)","平均_孔隙度(%)","构造复杂度","顶底板岩性",
