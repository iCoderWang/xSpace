        ...
        public List<TargetEntity> Assign(List<TargetEntity> list, DataTable dt)
        {
            foreach (DataRow row in dt.Rows)
            {
                list.Add(new TargetEntity()
                {
                    ...
                    //定义字段 地质资源量 (万亿方) geological resource
                    tgt_Att_Para_Gr = row[5].ToString(),

                    //定义字段 富有机质页岩厚度范围(m) Thinckness Range Riching Organic Matters Shale
                    tgt_Geo_Para_TrRoms = row[6].ToString(),

                    //定义字段 Toc 总有机碳
                    tgt_Geo_Para_Toc = row[7].ToString(),

                    //定义字段 Kt 有机质类型 Kerogen Type
                    tgt_Geo_Para_Kt = row[8].ToString(),

                    //定义字段 Ro 有机质成熟度
                    tgt_Geo_Para_Ro = row[9].ToString(),

                    //定义字段 有效圈定面积(km2) Effective Area
                    tgt_Geo_Para_Ea = row[10].ToString(),

                    //定义字段 含气量 (m3/t) Gas Content
                    tgt_Geo_Para_Gc = row[11].ToString(),

                    //定义字段 资源丰度 (108 m3/km2) Resource richness
                    tgt_Geo_Para_Rr = row[12].ToString(),

                    //定义字段 孔隙度 (%) Porosity
                    tgt_Geo_Para_Por = row[13].ToString(),
                    ...
                    //定义字段 埋深范围(m)  Depth Range 
                    tgt_Eng_Para_Dr = row[19].ToString(),

                    //定义字段 压力系数 Pressure Coefficient (Factor)
                    tgt_Eng_Para_Pc = row[20].ToString(),

                    //定义字段 渗透率 Permeability
                    tgt_Eng_Para_Per = row[21].ToString(),

                    //定义字段 裂缝发育程度 Fracture Development Degree
                    tgt_Eng_Para_Fdd = row[22].ToString(),

                    //定义字段 主应力差异系数 Principle Stress Diversity Coefficient (Factor)
                    tgt_Eng_Para_Psdc = row[23].ToString(),

                    //定义字段 脆性矿物含量 Brittle Mineral Content
                    tgt_Eng_Para_Bmc = row[24].ToString(),

                    //定义字段 水系 Drainage System
                    tgt_Eng_Para_Ds = row[25].ToString(),

                    //定义字段 区域勘探程度 Local Exploring Degree
                    tgt_Eng_Para_Led = row[26].ToString(),

                    //定义字段 市场气价 Gas Price
                    tgt_Mkt_Para_Gp = row[27].ToString(),

                    //定义字段 市场需求 Demand
                    tgt_Mkt_Para_Dmd = row[28].ToString(),

                    //定义字段 交通设施 Transport Utility
                    tgt_Mkt_Para_Tu = row[29].ToString(),

                    //定义字段 管网条件 Pipe Net
                    tgt_Mkt_Para_Pn = row[30].ToString(),

                    //定义字段 地表地貌 Suface and Geography
                    tgt_Mkt_Para_Sg = row[31].ToString()
                });
